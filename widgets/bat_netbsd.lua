...
local function worker(format, warg)
    ...
    local bat_info = {}
    local f = io.popen("envstat -d " .. helpers.shellquote(battery))
    ...
    -- current state
    local state
    if bat_info["ACIN present"] == "TRUE" and bat_info["charging"] == "FALSE" then
        state =  "↯"
    elseif bat_info["charging"] == "TRUE" then
        state = "+"
    elseif bat_info["charge state"] == "NORMAL" then
        state = "-"
    else
        state = "⌁"
    end
    ...
    local percent = tonumber(string.match(bat_info["battery percent"], "[%d]+"))

    -- use remaining (charging or discharging) time calculated by envstat
    local time = "∞"
    ...
    local rate = "N/A"
    -- if bat_info["charging"] == "TRUE" then
    --   rate = bat_info["battery charge current"]
    -- else
    --   rate = bat_info["battery discharge current"]
    -- end
...
return setmetatable(bat_netbsd, { __call = function(_, ...) return worker(...) end })
